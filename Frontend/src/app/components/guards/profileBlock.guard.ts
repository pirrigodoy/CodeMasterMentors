...
export class profileBlock implements CanActivate {
  ...
  canActivate(
    ...
    const currentAdvertisemetnId = localStorage.getItem('user_id'); // Obtener el ID de usuario autenticado
    ...
    if (!hasAccess) {
      ...
      const url = `/mis-anuncios/${currentAdvertisemetnId}`;
